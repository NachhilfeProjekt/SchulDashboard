...
export const setButtonPermissions = async (buttonId: string, permissions: ButtonPermission[]) => {
...
export const sendBulkEmails = async (recipients: {email: string, name: string}[], templateId: string) => {
  const response = await api.post('/emails/send-bulk', { recipients, templateId });
  return response.data;
};
